...
function getCheklist(plnaeId, plotId, ref) {
    ...
    const items = ((_a = collection.find(({ uid }) => uid === ref)) === null || _a === void 0 ? void 0 : _a.items) || [];
    const index = collection.findIndex(({ uid }) => uid === ref) || 0;
    const group = groups_json_1.default[index] || groups_json_1.default[0];
    const data = Object.assign(Object.assign({}, group), { uid: `${plnaeId}-${plotId}-${ref}`, startDate: (0, date_1.getDateString)((group === null || group === void 0 ? void 0 : group.startDate) || ""), finishDate: (0, date_1.getDateString)((group === null || group === void 0 ? void 0 : group.finishDate) || "") });
    const pieceworkList = items.map((key, index) => {
        let item = piecework_json_1.default[index];
        return Object.assign(Object.assign({}, item), { name: key, editable: true, incidents: item.status === "in-progress" ? (0, pieceworker_services_1.getInicidets)(item.incidents) : [], status: item.status === "in-progress"
